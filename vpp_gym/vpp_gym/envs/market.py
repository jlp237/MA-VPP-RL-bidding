...
def simulate_market(self, action_dict):
    """
    Simulates the market by collecting the bids and running the auction for each slot.

    Args:
    action_dict (dict): A dictionary containing the action taken by the agent, including the bid size and bid price for each slot.
    """

    auction_bids = self.bids_df[self.market_start : self.market_end]
    logging.debug(
        "log_step: " + str(self.logging_step) + " slot: " + "None" + " auction_bids = "
    )
    ...
    logging.info(
        ...
        + " slot: "
        + "None"
        + " Bid Submission time (D-1) = %s" % (self.bid_submission_time)
    )
    logging.info(
        "log_step: "
        + str(self.logging_step)
        + " slot: "
        + "None"
        + " Gate Closure time (D-1) = %s" % (self.gate_closure)
    )
    logging.info(
        "log_step: "
        + str(self.logging_step)
        + " slot: "
        + "None"
        + " Historic Data Window: from %s to %s "
        % (self.historic_data_start, self.historic_data_end)
    )
    logging.info(
        "log_step: "
        + str(self.logging_step)
        + " slot: "
        + "None"
        + " Forecast Data Window: from %s to %s "
    ...
    for slot in range(0, len(self.slot_date_list)):
        ...
        logging.info(
            ...
            + " slot: "
            + str(slot)
            + " Current Slot Time: (D) = %s" % (slot_date)
        )
        slot_bids = (
            auction_bids[slot_date:slot_date]
        ...
        logging.info(
            ...
            + " slot: "
            + str(slot)
            + " agents_bid_size = %s" % (agents_bid_size)
        )
        logging.info(
            "log_step: "
            + str(self.logging_step)
            + " slot: "
            + str(slot)
            + " agents_bid_price = %s" % (agents_bid_price)
        )
        ...
        if agents_bid_size == 0.0:
            logging.info(
                ...
                + " slot: "
                + str(slot)
                + " no participation in market of agent"
            )
            ...
            if agents_bid_price > settlement_price_DE:
                ...
                unique_country_bids = list(
                    {v["country"]: v for v in slot_bids_list}.values()
                )
                ...
                slot_bids_list_sorted_by_price = sorted(
                    slot_bids_list, key=lambda x: x["price"]
                )
                ...
                slot_bids_filtered_size_sum = sum(
                    [bid["size"] for bid in slot_bids_filtered]
                )
                ...
                if agents_bid_size >= slot_bids_filtered_size_sum:
                    ...
                    for bid in range(0, len(slot_bids_filtered)):
                        logging.debug(
                            ...
                            + str(slot)
                            + " bid size = "
                            + str(slot_bids_filtered[-(bid + 1)]["size"])
                        )
                        logging.debug(
                            "log_step: "
                            + str(self.logging_step)
                            ...
                            + str(slot)
                            + " bid price = "
                            + str(slot_bids_filtered[-(bid + 1)]["price"])
                        )
                        ...
                        logging.debug(
                            ...
                            + " slot: "
                            + str(slot)
                            + " accumulated_replaced_capacity = "
                            + str(accumulated_replaced_capacity)
                        )
                        ...
                        if accumulated_replaced_capacity >= agents_bid_size:
                            ...
                            if slot_bids_filtered[-(bid + 1)]["indivisible"] is False:
                                logging.debug(
                                    ...
                                    + " slot: "
                                    + str(slot)
                                    + " bid is divisible, so current bids price is new settlement price"
                                )
                                new_settlement_price_DE = slot_bids_filtered[
                                    -(bid + 1)
                                ]["price"]
                                logging.info(
                                    "log_step: "
                                ...
                                logging.debug(
                                    ...
                                    + " slot: "
                                    + str(slot)
                                    + " bid is INDIVISIBLE, so move one bids further is new settlement price"
                                )
                                accumulated_replaced_capacity -= bid_capacity
                                continue
        ...
        logging.info(
            ...
            + "\n"
            + " \n".join(
                "slot won: \t{}".format(k) for k in self.delivery_results["slots_won"]
            )
        )
        logging.info(
            "log_step: "
        ...
        logging.info(
            ...
            + " \n".join(
                "price: \t{}".format(k)
                for k in self.delivery_results["slot_settlement_prices_DE"]
            )
        )
