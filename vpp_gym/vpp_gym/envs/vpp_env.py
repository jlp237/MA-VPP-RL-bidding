...
class VPPBiddingEnv(Env):
    ...
    def __init__(self,
        ...
        # replace self.asset_data_total with self.asset_data_FCR_total
        ...
        self.size_scaler.fit(np.array([0., self.asset_data_FCR_total.max()]).reshape(-1, 1))  # type: ignore
        ...
        self.price_scaler.fit(np.array([0. , self.market_results["DE_SETTLEMENTCAPACITY_PRICE_[EUR/MW]"].max()]).reshape(-1, 1))  # type: ignore
