...
def render(self, mode="human"):
    ...
    if self.env_type != "test":
        ...
        price_plot.add_trace(
            go.Scatter(
                x=list(range(1, 7)),
                y=self.delivery_results["slot_settlement_prices_DE"],
                line_color="#3498DB",
                name="Market Price",
            )
        )
        price_plot.add_trace(
            go.Scatter(
                x=list(range(1, 7)),
                y=self.delivery_results["agents_bid_prices"],
                line_color="#D35400",
                name="Agents Price",
            )
        )
        price_plot.update_layout(
            width=300,
            height=400,
            title="Agents and Settlement Prices",
            xaxis_title="Slots",
            legend=dict(
                orientation="h",
                yanchor="bottom",
        ...
        capacity_plot.update_layout(
            width=300,
            height=400,
            title="Sold and Available Capacity",
            xaxis_title="4h-Slot (15min Res.)",
            legend=dict(
                orientation="h",
                yanchor="bottom",
                y=-0.6,
...
def list_generator(list_true):
    ...
    list_false: List[int] = [0] * 6
    list_none: List[int] = [0] * 6
