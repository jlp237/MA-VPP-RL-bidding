    ...
    for slot in range(0, 6):
        ...
        if agents_bid_size == 0.0:
            ...
            if agents_bid_price > settlement_price_DE:
                ...
                grouped_prices = [
                    x["settlement_price"]
                    for x in unique_country_bids
                ]
                cbmp = max(
                    set(grouped_prices), key=grouped_prices.count
                )
                ...
                for bid in range(0, len(slot_bids_filtered)):
                    bid_capacity = slot_bids_filtered[-(bid + 1)][
                        "size"
                    ]
