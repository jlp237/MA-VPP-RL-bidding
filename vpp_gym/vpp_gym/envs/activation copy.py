...
def check_activation_possible(agent_bid_size, vpp_total_step):
    ...
    # 2. Probability of maximum activation Amount (67,5 % :  0 - 5 %  Capacity , 
    # 23 % : 5 - 10 % , 6% : 10-15% and so on
    ...
    if abs(capacity_to_activate) > vpp_total_step:
        ...
        not_delivered_capacity += vpp_total_step * (1 - propab_of_activation)
    # If activation POSSIBLE as enough capacity is available:
    else:
        # Activation possible, but some capacity could not be fully provided
...
def simulate_activation(slot, vpp_total_slot, bid_sizes_per_slot):
    ...
    for time_step in range(0, 16):
        ...
        (
            activation_possible,
            not_delivered_energy,
        ) = check_activation_possible(agent_bid_size, vpp_total_slot[time_step])
